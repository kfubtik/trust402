...
if (targetUrl && args.local !== true) {
  const report = await postRemoteJson(targetUrl, "/api/operator/readiness", payload);
  console.log(JSON.stringify({
    ...report,
    remoteContext: {
      baseUrl: targetUrl.replace(/\/$/, ""),
      source: "/api/operator/readiness"
    },
    localProbeContext: localContext({ payload, githubCliAuthenticated, vercelProjectLinked })
  }, null, 2));
  if (args.strict === true && report.status !== "ready-for-live-evidence-window") process.exit(1);
  process.exit(0);
}

const report = operatorReadiness(payload, { config });

console.log(JSON.stringify({
  ...report,
  localContext: localContext({ payload, githubCliAuthenticated, vercelProjectLinked })
}, null, 2));
if (args.strict === true && report.status !== "ready-for-live-evidence-window") process.exit(1);
...
function localContext({ payload, githubCliAuthenticated, vercelProjectLinked }) {
  return {
    ...
    note: "Production readiness above comes from the remote API; this local context only describes this workstation."
  };
}
...
function localAgentcashPolicyProbe(payload) {
  ...
  const result = evaluateLocalAgentcashPolicyForLive({
    ...
    includeRefillLive: payload.includeAutoRefill === true
  });
