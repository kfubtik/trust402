...
- Vercel Cron calls two daily jitter slots:
  `/api/cron/daily-autonomous/morning` at `10 1 * * *` and
  `/api/cron/daily-autonomous/evening` at `47 13 * * *`. The agent executes
  only the pseudo-randomly selected slot for that date and skips the other.
  Configure `CRON_SECRET` before enabling `TRUST402_DAILY_AUTONOMY_ENABLED=true`;
  live daily autonomy also requires `TRUST402_DAILY_AUTONOMY_MODE=live`,
